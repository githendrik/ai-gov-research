...
## The `page.html` reserved-name gotcha

**Never create `layouts/_default/page.html`.** Since Hugo v0.146 `page.html` is the canonical template name for regular pages and **outranks `single.html`** in the lookup order. Adding one silently hijacks every post page — no build error, no warning, posts just quietly lose their hero image, date, tags and back-link while the homepage (which uses `index.html`) still looks correct.

This happened in commit `92f68bb` ("add privacy page"): a minimal `page.html` was added for the privacy page and broke the hero on all posts until `bbc6d99` renamed it to `plain.html`. One-off page layouts must use a non-reserved name and be opted into explicitly via `layout: <name>` in the page's frontmatter — as `content/{en,de}/privacy.md` do with `layout: plain`.

**Diagnostic:** if a post page renders a bare `<h1>` with no `.post-hero`, check `ls layouts/_default/` for a `page.html` before suspecting the transformer or the image pipeline.
...
- Don't create `layouts/_default/page.html` — it outranks `single.html` and silently breaks post heroes (see the reserved-name gotcha above).
